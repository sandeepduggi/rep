...
public class WrSearchResultsPage extends BasePage<WrSearchResultsPage> {
	...
	public WrSearchResultsPage() {
		super(Browsers.getBrowser());
		driver = Browsers.getBrowser();
		currentPageUrl("wyndham");
	}

	public WrSearchResultsPage openPage() throws Exception {
		return super.openPage(WrSearchResultsPage.class);
	}
...
//		}
	...
	public void ClickFirstAvailableButton() throws Exception {
		...
		for (int j = 0; j <= prop.size(); j++) {
			m++;
			if (n != 0) {
				AutomationUtils.scroll(driver, 0, 190);
			}
			n++;
			element = prop.get(j);
			if (m < prop.size()) {
	...
	public String SearchResult_Errorvalidation() throws Exception {
		...
		String Error1 = driver
				...
				.getText();
		if (!Error1.contains("hotels were found that match your criteria.")) {
			Assert.assertTrue(false);
		}
